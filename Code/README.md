...
## Quickstart (what the professor runs)
...
Notes:
- uses a **custom CNN** (the same family as the project’s custom CNN baseline),
...
### Custom CNN (your implementation)
...
The plots you generated (F1 ranking, radar comparison, fold variance, learning curves, confusion matrices) are produced from:
- saved experiment results (JSON/CSV) and
- `visualization.py` helpers.

If you already have a `results_*.json` / `results_*.csv`, you can regenerate plots by running the visualization script you used (or re-running `main.py`).
